...
{
  request p = q->req;
  printf("\t\tp->operation=%d, block_id=%d, pid=%d, error=%d, num_waiting=%d \n",p.operation, p.block_id, p.pid, p.error, p.num_waiting);
}
...
void queue_destroy ( struct queue *q )
{
        ...
        if ( NULL == q )
           {
              fprintf(stderr,
                      "[%s:%d]: no queue to destroy.\n",
                      __FILE__, __LINE__);
              return;
           }

        if ( ((NULL == q->head) && (NULL != q->tail)) ||
             ((NULL != q->head) && (NULL == q->tail)) )
           {
              fprintf(stderr,
                     "[%s:%d]: There is something seriously wrong with your assignment of head/tail to the list.\n",
                      __FILE__, __LINE__);
              return;
           }
 ...
 *
 * NOTE: the element is enqueued by value (the functions makes a fi o the element)
 * not by reference.
 *
 */
struct queue* enqueue ( struct queue* s, request * req )
{
   ...
   if ( NULL == s )
      {
         fprintf(stderr,
                 "[%s:%d]: queue not initialized\n",
                 __FILE__, __LINE__);
         return s;
      }

   if ( ((NULL == s->head) && (NULL != s->tail)) ||
        ((NULL != s->head) && (NULL == s->tail)) )
      {
         fprintf(stderr,
                "[%s:%d]: There is something seriously wrong with your assignment of head/tail to the list.\n",
                 __FILE__, __LINE__);
...
int is_queue_empty ( struct queue* s )
{
    ...
    if (NULL == s)
       {
          fprintf(stderr,
                  "[%s:%d]: queue not initialized (and empty because that)\n",
                  __FILE__, __LINE__);
          return 1;
       }
...
request * find_remove_request ( struct queue* s, int block_id, int operation) {
  request *req = find_request(s, block_id, operation);
  remove_request(s, req);
  return req;
}
...
struct queue* enqueue_ordered ( struct queue* s, request * req )
{
   ...
   fprintf(stderr,
  	   "[%s:%d]: no place to be inserted, enqueue_ordered has failed\n",
	   __FILE__, __LINE__);
   return NULL;
}
...
int remove_request ( struct queue* q, request * req )
{
   ...
   fprintf(stderr,
	  "[%s:%d]: Element to be removed was not found in the queue.\n",
	   __FILE__, __LINE__);
   return -1;
}
...
request * next_request ( struct queue* s, request * req )
{
  ...
  request * find_request_similar (struct queue* s, int block_id, int operation) {
   ...
   while (NULL != u)
   {
      if ((u->req.block_id == block_id) && (u->req.operation == operation) && u->req.num_waiting < 9) {
        struct queue_element* v = u->next;
        while(NULL != v) {
          if (v->req.block_id == block_id && v->req.operation != operation) {
            // there is another request with different operation. Let's find another request
            u = v->next;
            continue;    
          }
        v = v->next;
